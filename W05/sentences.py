...
def main():
    focused_noun_determiner = input('Enter if your focused noun is singular or plural.  1 for singular, 2+ for plural.\n')
    focused_scenario_time = input('Enter if the scenario is "past", "present", or "future".\n')
    chance = [1,2]
    secondary = random.choice(chance)
    if secondary == 1:
        secondary_noun_determiner = input('Enter if the secondary noun is singular or plural.  1 for singular, 2 for plural.\n')
        full_phrase = (get_determiner(focused_noun_determiner)).capitalize() + ' ' + get_noun(focused_noun_determiner) + ' ' +get_verb(focused_noun_determiner,focused_scenario_time) + ' ' + get_prepositional_phrase(secondary_noun_determiner) + '.'
    else:
        full_phrase = (get_determiner(focused_noun_determiner)).capitalize() + ' ' + get_noun(focused_noun_determiner) + ' ' +get_verb(focused_noun_determiner,focused_scenario_time) + '.'
    print(full_phrase)
